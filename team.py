...
def getTeamJSON(team_divs):
    ...
    with open('./outputs/team.json', 'w') as outfile:
        json.dump(team_json, outfile, indent=4)

def getTeamProfileLinks(team_divs):
    for team_div in team_divs:
        # get the name of the team
        team_name = team_div.find('h2', attrs={'class': 'section-header'}).text
        ...
        # get the link to each member's profile
        for team_member in team_members:
            team_member_link = team_member['href']
            team_member_links.append(team_member_link)
    
    with open('./assets/profile-links.txt', 'w') as outfile:
...
if __name__ == '__main__':
    ...
    # getTeamJSON(team_divs)

    getTeamProfileLinks(team_divs)
